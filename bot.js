...
exports.start = function(SETUP) {
 ...
 // const BOT_TOKEN = SETUP.BOT_TOKEN;
  ...
  const UpdateEmbed = function() {
    ...
    .setAuthor("HomeTown Roleplay Server Status", "https://i.imgur.com/FUOmFCv.png")
    .setColor(0x2894C2)
  ...
  const updateMessage = function() {
    getVars().then((vars) => {
      getPlayers().then((players) => {
        ...
        let embed = UpdateEmbed()
        .addField('Server Status','🛑 In Onderhoud',true)
  ...
  bot.on('ready',() => {
    ...
    bot.user.setActivity('HomeTown Roleplay',{'url':'https://www.twitch.tv/','type':'STREAMING'});
  ...
  bot.on('message',(message) => {
    if (!message.author.bot) {
      if (message.member) {
        ...
        if (message.channel.id === MEDEDELING_CHANNEL) {
          let embed = new Discord.RichEmbed()
          .setAuthor(message.member.nickname ? message.member.nickname : message.author.tag,message.author.displayAvatarURL)
          .setColor(3066993)
        ...
        if (message.channel.id === DISCORDS_CHANNEL) {
          let embed = new Discord.RichEmbed()
          .setAuthor(message.member.nickname ? message.member.nickname : message.author.tag,message.author.displayAvatarURL)
          .setColor(15844367)
          .setTitle('Discord Suggestie')
          .setDescription(message.content)
          .setTimestamp(new Date());
          message.channel.send(embed).then((message) => {
            const sent = message;
